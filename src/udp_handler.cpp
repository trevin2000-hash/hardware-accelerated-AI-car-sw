...
{
    // nothing will happen if you use the defualt constructor, so dont use it
    this->ip = "";
    this->port = 0;
    this->sockfd = 0;
}
...
bool udp_handler::send(uint16_t right_duty, uint16_t left_duty, uint16_t servo_period)
{
    ...
    if (this->ip == "")
    {
        std::cerr << "You are using the default constructor or udp_handler, bad things will happen";
        return false;
    }
    else
    {
